      ...
      <div className="flex flex-wrap items-center gap-y-2" aria-label={`Flow: ${nodes.join(" to ")}`} role="img">
        {nodes.map((n, i) => (
          <div key={n} className="flex items-center">
            <div className="shrink-0 border border-line bg-panel px-2 py-1.5 text-center font-mono text-[9px] tracking-wider text-fg/85 transition-colors duration-300 group-hover:border-accent/40">
              {n.toUpperCase()}
            </div>
            {i < nodes.length - 1 && (
              <span className="flow-h-connector mx-0.5 w-3 shrink-0" aria-hidden="true" />
            )}
          </div>
        ))}
      </div>
